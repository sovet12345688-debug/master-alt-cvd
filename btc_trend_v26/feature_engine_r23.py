...
def build_live_features_r23(now: datetime | None = None) -> dict:
    now = now or datetime.now(timezone.utc)
    payload = build_live_features_r22(now)
    payload["schema_version"] = "1.4"
    payload["engine_id"] = "BTC_TREND_V26_FEATURE_ENGINE_R23"
    ...
    try:
        ...
        if candidate.get("status") == "SHADOW_CANDIDATE":
            features["VOLUME_WAVE"] = candidate["feature"]
            ...
            payload["volume_wave_shadow"] = {"status": "UNAVAILABLE"}
...
def main() -> int:
    p = argparse.ArgumentParser(description="MASTER BTC TREND V2.6 R2.3 feature engine with Volume/Wave shadow")
    p.add_argument("--output", type=Path, default=HERE / "output" / "latest_features_r23.json")
    args = p.parse_args()
    try:
        payload = build_live_features_r23()
    except Exception as exc:
        payload = {
            "schema_version": "1.4",
            "engine_id": "BTC_TREND_V26_FEATURE_ENGINE_R23",
            "status": "VALIDATION_FAIL",
            "error": f"{type(exc).__name__}: {exc}",
