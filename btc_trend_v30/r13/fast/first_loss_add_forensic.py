...
def main() -> None:
    ...
    if int(x["first_route"].isna().sum()) or int(x["first_confirm_time_leg"].isna().sum()):
        raise SystemExit("FIRST ENTRY LINKAGE FAILED: route/confirm time missing")
    ...
    # Reproduce the legacy first_success=false slice exactly, but audit its one unresolved member.
    x = x[x["first_status"].ne("WIN")].copy()
    if len(x) != 45:
        raise SystemExit(f"POPULATION IDENTITY FAILED: expected 45 non-success ADD legs, got {len(x)}")
    strict_loss = x[x["first_status"].eq("LOSS")].copy()
    unresolved = x[x["first_status"].eq("UNRESOLVED")].copy()
    if len(strict_loss) != 44 or len(unresolved) != 1:
        raise SystemExit(f"FIRST STATUS AUDIT FAILED: strict_loss={len(strict_loss)} unresolved={len(unresolved)}")
    ...
    x.loc[known_resolved, "add_after_first_resolved"] = (
        x.loc[known_resolved, "confirm_time"] > x.loc[known_resolved, "first_resolved_time_leg"]
    )
    ...
    add_r = pd.to_numeric(x["realized_unit_R"], errors="coerce")
    x["add_win"] = add_r > 0
    winners = x[x["add_win"]].copy()
    losers = x[~x["add_win"]].copy()
    if len(winners) != 3 or len(losers) != 42 or float(add_r.sum()) != -33.0:
        raise SystemExit(f"LEGACY SLICE OUTCOME FAILED: wins={len(winners)} losses={len(losers)} sumR={add_r.sum()}")

    strict_add_r = pd.to_numeric(strict_loss["realized_unit_R"], errors="coerce")
    strict_stats = {
        "n": int(len(strict_loss)),
        "wins": int((strict_add_r > 0).sum()),
        "losses": int((strict_add_r < 0).sum()),
        "sum_R": float(strict_add_r.sum()),
        "mean_R": float(strict_add_r.mean()),
    }
    ...
    case_cols = [c for c in case_cols if c in x.columns]

    out = {
        "status": "FAST_DIAGNOSTIC_ONLY_NOT_PROMOTION_EVIDENCE",
        "slice": "FIRST_1H_NON_SUCCESS_THEN_4H_ADD_45_AUDITED",
        "population_identity": {
            "legacy_non_success_then_add_n": int(len(x)),
            ...
            "legacy_sum_R": float(add_r.sum()),
            "legacy_mean_R": float(add_r.mean()),
            "strict_first_loss_subset": strict_stats,
            "correction": "The prior 45 count was first_success=false, not 45 confirmed first losses. It contains 44 confirmed first losses plus 1 unresolved first entry."
        },
        "unresolved_first_entry_case": [{k: clean(v) for k, v in row.items()} for row in unresolved[case_cols].to_dict(orient="records")],
        "pre_add_numeric": {
            "legacy45": numeric_summary(x, ["quality_score", "independent_reasons", "safety_score", "add_delay_h"]),
        ...
        "post_first_entry_timing": {
            "known_first_resolved_n": int(known_resolved.loc[x.index].sum()) if len(x) else 0,
            "after_first_resolved_true_n": int((x["add_after_first_resolved"] == True).sum()),
            "route_changed_n": int(x["route_changed"].sum()),
            "route_changed_and_after_resolved_n": int((x["route_changed"] & (x["add_after_first_resolved"] == True)).sum()),
        },
    ...
    if args.csv_output:
        x[case_cols].to_csv(args.csv_output, index=False)
