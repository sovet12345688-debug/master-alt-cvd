...
정확히 1개 테이블. 세로축=지표, 가로축=거래소.
열=`지표 | Bitget | OKX`.
필수 행=`OI | CVD | Taker Buy/Sell | RSI | KDJ | 거래량`.
각 셀은 가능한 범위에서 `현재 수치/방향 → 최근 변화 → 가격구조와 결합한 의미`까지 상세 분석한다.
- OI: 증감과 가격 상승/하락 조합.
- CVD: 매수/매도 우위, 확대/축소, 다이버전스/흡수 가능성.
- Taker Buy/Sell: 적극매수/적극매도 비중과 편향.
- RSI: 현재 수준, 과매수/과매도, 모멘텀 변화, 실제 보이는 경우 다이버전스.
- KDJ: K/D/J 위치, 크로스, 반전/추세지속 가능성.
- 거래량: 현재 거래량 vs 화면의 평균/최근 구간, 돌파·반등·하락 신뢰도.
- 미제공/판독불가=N/A. 추정/다른 거래소 값으로 대체 금지.
표 바로 아래 반드시 `💡 핵심 요약:`으로 현재 매수/매도 수급 우세와 가장 중요한 확인 지표를 1~2문장으로 정리한다.
